...
bool is_in_env(char *to_add, char **env)
{
    ...
    while (env[i]) {
        ...
        if (ft_strcmp(temp1, temp2) == 0) {
            free(temp1);
            free(temp2); // Free temp2 here
            free(env[i]); // Free old env[i]
            env[i] = ft_strdup(to_add); // Ensure to free the old env[i] before overwriting
            return (true);
        }
        free(temp2); // Free temp2 in each loop iteration
        i++;
    }
    free(temp1); // Ensure temp1 is freed at the end
    return false;
}
